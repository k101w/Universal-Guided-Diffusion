...
os.environ['HF_HOME'] = "/data3/kewenwu/huggingface"
os.environ['HF_DATASETS_CACHE'] = "/data3/kewenwu/huggingface"
...
from torchvision import transforms, utils
import random
from helpers import OptimizerDetails
# import clip
import os
import inspect
...
def get_optimation_details(args):
    ...
    l_func = pipeline("text2text-generation", model="zhiqiulin/clip-flant5-xxl")
    # Load model directly


    l_func = AutoModelForSeq2SeqLM.from_pretrained("zhiqiulin/clip-flant5-xxl")
    for param in l_func.parameters():
        param.requires_grad = False

    l_func.to(device1).eval()
    # l_func = torch.nn.DataParallel(l_func).cuda()
...
def main():
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default="/data3/kewenwu/Universal-Guided-Diffusion/stable-diffusion-guided/sd-v1-4.ckpt",
    ...
    tokenizer = AutoTokenizer.from_pretrained("zhiqiulin/clip-flant5-xxl",cache_dir='/data3/kewenwu').to(device2)
    text = tokenizer(text)
    text = text.to(device0)
    root = "/data3/kewenwu/Universal-Guided-Diffusion/original_image/"
